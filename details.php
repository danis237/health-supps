<?php
 ...
 if(!isset($_GET['$pro_id'])){ //if the product active is id, then do this:
   ...
   $query  = "select * from pro_fav_like where user_name='$user_name' && product_id=$pro_id";// query for checking
	...
	$favlikeData = mysqli_fetch_assoc($data);
	$rows=mysqli_num_rows($data);// check number for rows
	if($rows > 0 ){
		$fav = $favlikeData['favorite'];
		$like = $favlikeData['like_pro'];
	}else{
		$fav = "";
		$like ="";
	}
        ...
        <div class="col-md-3"><!--colmd3 start-->
         ...
         <div class="col-md-9">
           <div class="row" id="productmain">
			...
			<div class='row'>
				 <div class='col-sm-6'>
      			<button class='btn btn-default' id='favorite' data-user='<?php  echo $_SESSION['username'];?>' data-proId='<?php echo $pro_id;?>'
              data-type='fav'><i class='fa fa-heart heart' style="color:<?php echo ($fav=="yes")?'red':'black'?>"></i> <span id='fav'> <?php echo
              ($fav=="yes")?'Favorite':'Add to Favorite'?></span>
            </button>
			   </div>
			   <div class='col-sm-6'>
				     <button class='btn btn-default pull-right' data-user='<?php  echo $_SESSION['username'];?>' id='like' data-proId='<?php echo $pro_id;?>'
               data-type='like'><i class='fa fa-thumbs-up like' style="color:<?php echo ($like=="yes")?'blue':'black'?>"></i> <span id='like_pro'>
              <?php echo ($fav=="yes")?'Liked':'Like'?> </span></button>
			   </div>
      </div>
               </div><!--box end-->
